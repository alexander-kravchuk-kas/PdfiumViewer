...
namespace IronPdf.Pdfium
{
    internal class PdfFile : IDisposable
    {
        ...
        private IList<PdfRectangle> GetTextBounds(IntPtr textPage, int page, int index, int matchLength)
        {
            ...
            for(int i = 0; i < matchLength; i++)
            {
                    ...
                    AreClose(lastBounds.Value.Bottom, bounds.Bottom))
                {
                    ...
                    lastBounds = new RectangleF(lastBounds.Value.Left, top, bounds.Right - lastBounds.Value.Left, bottom - top);
        ...
        public DateTime? GetMetaTextAsDate(string tag)
        {
            ...
            Regex dtRegex =
                new Regex(@"(?:D:)(?<year>\d\d\d\d)(?<month>\d\d)(?<day>\d\d)(?<hour>\d\d)(?<minute>\d\d)(?<second>\d\d)(?<tz_offset>[+-zZ])?(?<tz_hour>\d\d)?'?(?<tz_minute>\d\d)?'?");
        ...
        public Rectangle RectangleFromPdf(int page, RectangleF rect)
        {
            using(var pageData = new PageData(_document, _form, page))
            {
                ...
                NativeMethods.FPDF_PageToDevice(pageData.Page, 0, 0, (int)pageData.Width, (int)pageData.Height, 0, rect.Right, rect.Bottom, out var deviceX2, out var deviceY2);

                return new Rectangle(deviceX1, deviceY1, deviceX2 - deviceX1, deviceY2 - deviceY1);
            }
        }
        ...
        public RectangleF RectangleToPdf(int page, Rectangle rect)
        {
            using(var pageData = new PageData(_document, _form, page))
            {
                ...
                NativeMethods.FPDF_DeviceToPage(pageData.Page, 0, 0, (int)pageData.Width, (int)pageData.Height, 0, rect.Right, rect.Bottom, out var deviceX2, out var deviceY2);

                return new RectangleF((float)deviceX1, (float)deviceY1, (float)(deviceX2 - deviceX1), (float)(deviceY2 - deviceY1));
            }
        }
                ...
                using(var pageData = new PageData(_document, _form, page))
                {
                    ...
                    try
                    {
                        while(NativeMethods.FPDFText_FindNext(handle))
                        {
                            ...
                            matches.Add(new PdfMatch(match, new PdfTextSpan(page, index, matchLength), page));
                        }
                    }
                    finally
                    {
