...
[Description("Occurs when a link in the pdf document is clicked.")]
        ...
        private Rectangle BoundsFromPdf(PdfRectangle bounds, bool translateOffset)
        {
            ...
            var translated = Document.RectangleFromPdf(bounds.Page, bounds.Bounds);
            ...
            return new Rectangle(pageBounds.Left + offset.Width + Math.Min(topLeft.X, bottomRight.X), pageBounds.Top + offset.Height + Math.Min(topLeft.Y, bottomRight.Y), Math.Abs(bottomRight.X - topLeft.X), Math.Abs(bottomRight.Y - topLeft.Y));
        }
        ...
        {
            return new Rectangle(0, 0, ((scrollbars == ScrollBars.Vertical) || (scrollbars == ScrollBars.Both)) ? (Width - SystemInformation.VerticalScrollBarWidth) : Width, ((scrollbars == ScrollBars.Horizontal) || (scrollbars == ScrollBars.Both)) ? (Height - SystemInformation.HorizontalScrollBarHeight) : Height);
        }
        ...
        private void RebuildPageCache()
        {
            ...
            for(int page = 0; page < Document.PageSizes.Count; page++)
            {
                ...
                pageCache.Bounds = new Rectangle(thisLeftOffset + ShadeBorder.Size.Left + PageMargin.Left, offset + ShadeBorder.Size.Top + PageMargin.Top, width, height);
                pageCache.OuterBounds = new Rectangle(thisLeftOffset, offset, width + ShadeBorder.Size.Horizontal + PageMargin.Horizontal, height + ShadeBorder.Size.Vertical + PageMargin.Vertical);
        ...
        private void ResetFromRotation()
        {
            ...
            SetDisplayRectLocation(new Point((int)(DisplayRectangle.Width * offsetX), (int)(DisplayRectangle.Height * offsetY)));
        }
        ...
        private Point TranslatePointFromPdf(Size size, SizeF pageSize, PointF point)
        {
            ...
            return new Point((int)((point.X / pageSize.Width) * size.Width), (int)((point.Y / pageSize.Height) * size.Height));
        }
        ...
        private PointF TranslatePointToPdf(Size size, SizeF pageSize, Point point)
        {
            ...
            return new PointF((((float)point.X) / size.Width) * pageSize.Width, (((float)point.Y) / size.Height) * pageSize.Height);
        }
        ...
        protected override Rectangle GetDocumentBounds()
        {
            ...
            if((DisplayRectangle.Width > ClientSize.Width) ||
        ...
        protected override void SetZoom(double zoom, Point? focus)
        {
            ...
            if(focus.HasValue)
            {
                ...
                location = new Point(focus.Value.X - bounds.X, focus.Value.Y - bounds.Y);
            }
            else
            {
            ...
            var newLocation = new Point((int)(location.X * (zoom / oldScale)), (int)(location.Y * (zoom / oldScale)));

            SetDisplayRectLocation(new Point(DisplayRectangle.Left - (newLocation.X - location.X), DisplayRectangle.Top - (newLocation.Y - location.Y)), false);
        }
        ...
        public PdfRectangle BoundsToPdf(Rectangle bounds)
        {
            ...
            for(int page = 0; page < Document.PageSizes.Count; page++)
            {
                ...
                if(pageCache.OuterBounds.Contains(bounds.Location))
                {
                    ...
                    {
                        var topLeft = new Point(bounds.Left - pageCache.Bounds.Left, bounds.Top - pageCache.Bounds.Top);
                        var bottomRight = new Point(bounds.Right - pageCache.Bounds.Left, bounds.Bottom - pageCache.Bounds.Top);
                        ...
                        var translated = Document.RectangleToPdf(page, new Rectangle((int)translatedTopLeft.X, (int)translatedTopLeft.Y, (int)(translatedBottomRight.X - translatedTopLeft.X), (int)(translatedBottomRight.Y - translatedTopLeft.Y)));
        ...
        public Point PointFromPdf(PdfPoint point)
        {
            ...
            return new Point(pageBounds.Left + offset.Width + location.X, pageBounds.Top + offset.Height + location.Y);
        }
        ...
        public PdfPoint PointToPdf(Point location)
        {
            ...
            for(int page = 0; page < Document.PageSizes.Count; page++)
            {
                ...
                if(pageCache.OuterBounds.Contains(location))
                {
                    ...
                    {
                        location = new Point(location.X - pageCache.Bounds.X, location.Y - pageCache.Bounds.Y);
        ...
        public void ScrollIntoView(Rectangle rectangle)
        {
            ...
            if((rectangle.Top < 0) || (rectangle.Bottom > clientArea.Height))
            {
                ...
                SetDisplayRectLocation(new Point(displayRectangle.X, -offset));
            }
        }
        ...
        /// true.Note:This property will always return true for an instance of the <see
        /// cref="T:System.Windows.Forms.Form"/> class.
        /// </returns>
        /// <filterpriority>1</filterpriority><PermissionSet>
        /// <IPermission class="System.Security.Permissions.EnvironmentPermission, mscorlib, Version=2.0.3600.0,
        /// Culture=neutral, PublicKeyToken=b77a5c561934e089" version="1" Unrestricted="true"/><IPermission
